...
export default function OrderList({ orders }: OrderListProps) {
  ...
  const getCustomerName = (order: Order) => {
    // Check for shipping address name first (directly provided during checkout)
    if (order.shipping_address?.name) {
      return order.shipping_address.name;
    }

    // Then check for customer profile
    ...
    // Then check for email from profile
    if (order.profiles?.email) {
      // If no profile info, use email from auth
      return order.profiles.email.split("@")[0];
    }

    // Finally use metadata if available
    if (order.metadata?.customerName) {
      return order.metadata.customerName;
    }

    return "Unknown Customer";
  };

  // Add a method to get customer email
  const getCustomerEmail = (order: Order) => {
    // First check the customer_profiles
    if (order.customer_profiles?.email) {
      return order.customer_profiles.email;
    }

    // Then check auth profiles
    if (order.profiles?.email) {
      return order.profiles.email;
    }

    // Check metadata as fallback
    if (order.metadata?.customerEmail) {
      return order.metadata.customerEmail;
    }
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        ...
                        <div className="text-xs text-gray-500">
                          ...
                          {/* Using the function here instead of inline */}
                        </div>
                      </div>
                    </div>
                  </td>
